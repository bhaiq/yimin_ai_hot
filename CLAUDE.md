...
- **WeChat Work push**: `POST /api/push/daily` fetches department users via WeChat Work API, generates a unique token per user, sends textcard messages with `/d/:token` links. Visit tracking records first-click timestamp and IP. `GET /d/:token` redirects to `/#daily` after logging. Access token cached in `yimin_wx_token_cache`. Config: `WX_WORK_CORP_ID`, `WX_WORK_AGENT_ID`, `WX_WORK_SECRET`, `WX_WORK_PUSH_DEPT_IDS`, `PUBLIC_BASE_URL`.
