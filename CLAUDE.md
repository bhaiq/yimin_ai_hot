...
- **Personalized daily supplements**: `#subscriptions` lets an identified WeChat Work user select enabled `yimin_sources`. Department defaults live in `yimin_department_source_subscriptions`; personal `subscribed`/`muted` rows in `yimin_user_source_subscriptions` are merged over those defaults. `/api/daily/personal` filters existing public report items by the effective source set and removes URLs already cited in public report Markdown. It does not call DeepSeek. Push text uses the same effective subscription calculation.
...
- **Local SSO test identity**: `LOCAL_TEST_SSO_ENABLED=1`, `LOCAL_TEST_SSO_USER_ID`, `LOCAL_TEST_SSO_USER_NAME`, and optional comma-separated `LOCAL_TEST_SSO_DEPARTMENT_IDS` provide a development fallback only when `NODE_ENV !== production` and both the request host and remote address are loopback. A valid signed SSO cookie always takes precedence. Never enable this in production.
