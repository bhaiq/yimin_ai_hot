...
- **Department daily focus**: `/api/daily/department` uses only department IDs directly stored in the identified user's `departments_json`; it does not traverse `parent_id`. `POST /api/daily/departments/generate` pre-generates all real departments with configured default sources at concurrency 2 after the public report exists; it returns 409 when the public report is missing and 207 for partial failures. Each direct department is generated separately from its default subscribed sources and the current public daily report items. Results are cached in `yimin_department_daily_reports` by date/department and regenerated only when the source configuration or article/public-report input hash changes. Empty inputs skip DeepSeek; generation failures save a rule-based fallback.
