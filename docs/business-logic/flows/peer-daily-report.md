...
> 2026-08-12 决策更新：停止同行官网项目采集。同行日报从此只使用公众号文章；官网项目、变化事件和采集状态不进入 Prompt、报告、“今日无更新”或完整性提示。本文后续官网设计章节仅作为历史方案留档，不再执行。
...
| 来源链接 | 完整日报保留公众号原文链接；MD 简版只保留内部完整日报入口 |
| 老板速览 | 完整日报不单设“老板速览” |
| 报告尾部 | 不展示四种状态图例，不输出建议清单 |
| 官网采集 | 已放弃并停用；历史数据和兼容代码暂不删除，但不再运行或进入日报 |
