...
export const PEER_DAILY_PROMPT_VERSION = "peer-daily-v1";
export const PEER_SOURCE_ANALYSIS_PROMPT_VERSION = "peer-source-analysis-v1";
...
function inferCategory(source) {
  if (source.sourceType === "website_event") return "官网项目变化";
...
function inferImportance(source, category) {
  if (source.sourceType === "website_event") return "important";
  const text = `${source.title || ""} ${source.summary || ""} ${source.content || ""}`;
...
function summarizeWebsiteEvent(source) {
  const action = {
    added: "新增展示",
    changed: "出现实质变化",
    removed: "不再展示",
    reappeared: "恢复展示",
  }[source.eventType] || "发生变化";
  const fields = uniqueStrings(source.changedFields, 8, 80);
  const fieldText = fields.length ? `，涉及${fields.join("、")}` : "";
  return `${source.competitorName}官网的“${source.projectName || "相关项目"}”在本轮检测中${action}${fieldText}。`;
}
...
export function createFallbackSourceAnalysis(source) {
  ...
  const rawSummary = source.sourceType === "website_event"
    ? summarizeWebsiteEvent(source)
    : `${source.competitorName}发布《${source.title || "未命名文章"}》：${source.summary || cleanText(source.content, 180) || "正文主要围绕标题所示主题展开。"}`;
  return {
    sourceRef: source.sourceRef,
    competitorName: source.competitorName,
    category,
    importance,
    actionTitle: cleanText(source.title || source.projectName || "公开动态", 180),
    actionSummary: cleanText(rawSummary, 500),
    evidencePoints: uniqueStrings(
      source.sourceType === "website_event"
        ? [summarizeWebsiteEvent(source)]
  ...
  const append = (section, record) => {
    for (const sourceRef of record.sourceRefs || []) {
      ...
      items.push({
        ...
        websiteEventId: source.sourceType === "website_event" ? source.id : null,
        section,
