...
`PEER_WEBSITE_PROXY_URL` 会注入九个官网采集子进程，但不会影响 Node、WeRSS、
公众号任务或快照回传本机 API。`PEER_WEBSITE_REQUIRE_PROXY=1` 开启强制模式：
运行器先通过显式代理做出口预检；代理缺失或不可用时整轮停止，不会回退直连。
Playwright 站点同时向 Chromium 传入显式代理并禁用 QUIC。代理地址只接受带端口
的 loopback HTTP(S) 地址，避免意外连接外部未知代理。
...
| `PEER_WEBSITE_PROXY_URL` | 空 | 九家官网采集器共用的本机 HTTP(S) 代理 |
| `PEER_WEBSITE_REQUIRE_PROXY` | `0` | 设为 `1` 时代理缺失或预检失败即停止整轮 |
| `PEER_WEBSITE_PEER_A_PROXY_URL` 等 | 空 | 可选；仅覆盖指定同行的全局代理设置 |
