...
- 同行监控：抓取同行A–同行I的公众号文章并生成仅基于公众号内容的每日同行动态；历史官网项目数据保留但不再采集，也不进入日报

## 同行监控

`#peer-monitor` 是内部同行情报页面，与公共新闻、移民日报和市场素材的数据表完全隔离。

- 当前监控同行A–同行I，共 9 家公众号；官网采集已于 2026-08-12 停用，已有官网项目仅作历史数据保留
...
- 页面访问当前默认向所有访问者开放；将 `PEER_MONITOR_OPEN_ACCESS=0` 后重启服务，即可恢复企业微信身份白名单限制
- 关闭公开访问后，仅允许企业微信直属部门为 IOD 或 MD 的用户，以及指定 UserID `fanrui`；可通过 `PEER_MONITOR_USER_IDS` 和 `PEER_MONITOR_DEPARTMENT_NAMES` 追加逗号分隔的允许名单
...
以下官网能力仅作为历史兼容保留，不运行生产采集任务，也不进入同行日报。若未来重新启用，匿名种子命令为：
...
统一采集任务的历史说明见 `collectors/peer-web/README.md`。当前不得配置或运行下列生产命令：
...
只验证一家同行时传入 `competitor`，例如 `peer-b` 对应景鸿移民。单账号任务与全量任务分别记录；首次测试不要传 `refresh`，避免误重复扣费：

```bash
curl -X POST 'https://your-domain.example/api/peer-monitor/wechat/discover' \
  -H 'Authorization: Bearer your-cron-token' \
  -H 'Content-Type: application/json' \
  -d '{"date":"2026-08-07","competitor":"peer-b"}'
```
...
同行日报使用报告日前一日 06:30（含）到报告日 06:30（不含）的固定窗口，只纳入 `content_text` 非空的公众号文章。官网项目、官网变化事件和官网采集状态均不进入日报、Prompt、“今日无更新”计算或数据完整性提示。完整报告使用真实同行名称并保留文章链接；MD 简版不包含外部链接，只提供内部授权页面入口。无动作日期也会生成，公众号采集异常同行不会被误列为“今日无更新”。
